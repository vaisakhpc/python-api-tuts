    ...
    def get(self, request):
        ...
        try:
            ...
            es_query = {
                "query": {
                    "bool": {
                        "should": [
                            {"multi_match": {
                                "query": query,
                                "fields": ["mf_name^3", "type"],  # Boost 'mf_name' field
                            }},
                ...
                "sort": [
                    {"aum": {"order": "desc"}}  # Sort by AUM in descending order
                ],
            ...
            if is_integer:
                es_query["query"]["bool"]["should"].append({"term": {"mf_schema_code": int(query)}})
           ...
           # Return the paginated response with grouped results
            ...
            logger.warning(f"Error during Elasticsearch search: {e}. Falling back to database.")
            if isinstance(e, (ConnectionError, Exception)):
                logger.warning(f"Error during Elasticsearch search: {e}. Traceback: {traceback.format_exc()}")
